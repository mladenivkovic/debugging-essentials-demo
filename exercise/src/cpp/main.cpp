...
constexpr double CFL_factor = 0.9;
...
constexpr int nghosts = 2;
/*! Size of the box being simulated */
constexpr double boxsize = 2.;
...
int main(void){
  ...
  Grid grid = Grid(ncells_tot);

  /* Initialize default values */
  grid.first_init_cells(dx);

  /* Impose initial conditions */
  grid.setup_initial_conditions(ncells, nghosts, boxsize, a, dx);
  ...
  io_write_output(snapshot, step, t, grid, ncells, nghosts);
  ...
  while ( t < t_end && step < max_nr_steps){
    ...
    grid.apply_boundary_conditions(ncells, nghosts);

    /* First find the next time step size. */
    dt = solver_find_next_time_step_size(grid, ncells_tot, CFL_factor);
    ...
    if (t + dt > next_output_time){
      ...
      next_output_time += delta_t_output;
      write_output = true; /* flag that we're writing output now. */
    }

    /* Run the actual solver step */
    solver_step(grid, ncells, nghosts, dt);
